...
>
> The `lit` watcher advances a per-run cursor (`state.cursor` = last-seen `publication_date`) so each run only fetches papers newer than the last seen, closing the per-run-cap coverage gap.
